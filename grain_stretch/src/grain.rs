...
impl Grain {
  ...
  pub fn process(
    ...
    let position_a = self.position * 2.;
    ...
    let delay_out = delay_line
      ...
      .multiply(position_a * grain_fade)
      .add(
        delay_line
  ...
  pub fn set_parameters(&mut self, spray: f32, start_phase: f32) {
    let spray = fastrand::f32() * spray;
    self.phase = 0.;
    self.position = (1. - spray + start_phase) * 0.5;
    self.is_active = true;
  }
  ...
  fn get_grain_fade(&self, window_factor: f32) -> f32 {
    ...
    Self::apply_windowing_function(fade)
  }
  ...
  fn apply_windowing_function(x: f32) -> f32 {
    (1. - (x * PI).cos()) * 0.5
  }
