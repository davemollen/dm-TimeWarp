...
impl DmTimeWarp {
  ...
  pub fn write_set_file(&mut self, ports: &mut Ports) -> Result<(), AtomWriteError> {
    ...
    let mut property = writer.new_property(self.urids.patch.property, self.urids.atom.urid)?;
    property.set(self.urids.sample.into_general())?;

    let mut value = writer.new_property(self.urids.patch.value, self.urids.atom.path)?;
    value.append(&self.file_path)?;
